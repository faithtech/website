<div class="clearfix">
	<i id="btn-menu" class="fa fa-bars visible-xs" aria-hidden="true"></i>
	<h1 class="logo"><a href="index.php" title="Checkin+"><img src="public/img/logo.png" alt="Checkin+"></a></h1>
	<nav class="nav-main">
        <ul>
    		<li><a href="index.php#nav-1" class="btn-scroll trn">What is Checkin+ ?</a></li>
    		<li><a href="index.php#nav-2" class="btn-scroll trn">How to work?</a></li>
    		<li><a href="index.php#nav-3" class="btn-scroll trn">Our Services</a></li>
    		<li><a href="pricing.php" class="trn">Price List</a></li>
    		<li><a href="#nav-4" class="btn-scroll trn">About</a></li>
    		<li class="dropdown"><a href="#" id="dropdownLang" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true"><span class="trn">Lang</span> <span class="caret"></span></a>
    			<ul class="dropdown-menu" aria-labelledby="dropdownLang">
    				<li><a href="#" id="zhtw" class="btn-translate">正體中文</a></li>
    				<li><a href="#" id="en" class="btn-translate">English</a></li>
    			</ul>
    		</li>
    	</ul>
    </nav>
</div>